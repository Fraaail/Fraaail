...
export const AiChat: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      role: "model",
      text: "Greetings. I am the system AI. Ask me anything about @Fraaail's projects, skills, or configuration.",
      timestamp: Date.now(),
    },
  ]);
  ...
  return (
    <div className="p-4 h-full max-w-4xl mx-auto">
      <WindowFrame
        title="gemini-cli"
        isActive={true}
        className="h-full flex flex-col border-purple-500"
      >
        {/* Messages Area */}
          ...
          >
            ...
            {isLoading ? (
              ...
              <button
                type="submit"
                disabled={!input.trim()}
                className="text-neutral-500 hover:text-purple-400 disabled:opacity-30 transition-colors"
              >
                <Send size={18} />
              </button>
            )}
